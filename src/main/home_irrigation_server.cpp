...
int HomeIrrigationServer::server_init() {
    ...
    if(m_is_added) {   //If the device is added
        ...
        m_thread_objs->pump_thread_obj = new PumpThread(m_watering_record_helper, m_adc_hardware);
        m_thread_objs->pump_thread_obj->create_thread(m_server_info);
        ...
        m_thread_objs->soil_moisture_thread_obj = new SoilMoistureThread(m_adc_hardware, m_watering_record_helper);
        m_thread_objs->soil_moisture_thread_obj->create_thread();
    ...
    m_thread_objs->net_thread_obj = new NetworkingThread(m_server_info_database_helper, m_watering_record_helper, m_adc_hardware);
    m_thread_objs->net_thread_obj->create_thread(&m_is_added, m_server_id, m_server_info, m_thread_objs);
